...
class AdminSendToStudentsController extends GetxController {
  ...
  /// ================= Date / Time (عرض) =================
  RxString eventDay = "select_date".obs; // yyyy-MM-dd (محلي للعرض)
  RxString eventTime = "time".obs; // HH:mm (محلي للعرض)
  ...
  /// ================= Lookups =================
  List get colleges => app.colleges;
  List get programs => app.programs;
  List get levels => app.levels;
  List get subjects => app.subjects;
  ...
  void selectProgram(int id) {
    ...
    /// إذا اخترنا برنامج نلغي الكلية (حسب منطقك)
    collegeId = null;
  ...
  void pickTime(BuildContext context) async {
    // نخليها مثل ما عندك: لازم تاريخ أولاً
    if (_pickedDateLocal == null) {
  ...
  Future<void> sendNotification() async {
    ...
    try {
      ...
      final Map<String, dynamic> body = {
        ...
        if (collegeId != null) "college_id": collegeId.toString(),
      ...
      response.fold(
        ...
        (r) {
          if (r['status'] == true) {
            final msg = r['message'] ?? "تم إرسال الإشعار بنجاح";
